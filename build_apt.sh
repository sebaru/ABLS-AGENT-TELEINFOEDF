#!/bin/bash
# build_apt.sh -- Build DEB package for abls-agent-teleinfoedf via CPack
set -euo pipefail
...
echo "Building DEB package for abls-agent-teleinfoedf..."
...
if [[ "$CLEAN" == "true" ]]; then
  rm -f "$BUILD_DIR"/abls-agent-teleinfoedf*.deb
  rm -f "$ARTIFACT_DIR"/abls-agent-teleinfoedf*.deb
fi
...
while IFS= read -r deb_file; do
  if command -v dpkg-deb >/dev/null 2>&1; then
    ...
    if [[ "$pkg_name" == "abls-agent-teleinfoedf" ]]; then
  ...
  case "$(basename "$deb_file")" in
    *runtime*.deb|abls-agent-teleinfoedf-*.deb)
      runtime_deb="$deb_file"
      ;;
  esac
