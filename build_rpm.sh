...
echo "Building RPM package for abls-agent-teleinfoedf..."
...
rm -f "$BUILD_DIR"/abls-agent-teleinfoedf-*.rpm

pushd "$BUILD_DIR" >/dev/null
cpack -G RPM
popd >/dev/null

runtime_rpm=$(find "$BUILD_DIR" -maxdepth 1 -type f -name 'abls-agent-teleinfoedf-[0-9]*.rpm' | sort | tail -n 1)
